...
import {
  ArrowRight, Calendar, Clock, Tag, Globe,
  TrendingUp, Cpu, MessageSquare, User, Mail, ExternalLink,
} from "lucide-react";
...
export default function BlogShopifyPage() {
    ...
    <main className="min-h-screen bg-[#080316] text-foreground overflow-x-hidden selection:bg-primary/30">
      ...
      <section className="relative pt-28 pb-16 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 lg:px-10">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 text-xs text-white/35 mb-8">
            ...
            <span>/</span>
            <span className="text-white/55">Shopify for Small Business</span>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }} className="flex flex-wrap items-center gap-4 mb-6">
            ...
            <span className="flex items-center gap-1.5 text-white/35 text-xs"><Calendar className="w-3 h-3" /> 08 Jan 2025</span>
            <span className="w-px h-3 bg-white/15" />
            <span className="flex items-center gap-1.5 text-white/35 text-xs"><Clock className="w-3 h-3" /> 5 min read</span>
          </motion.div>
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.6 }} className="text-3xl md:text-5xl font-black text-white leading-[1.1] tracking-tight mb-6">
            ...
            {" "}Worth It for a<br />Small Business? <span className="text-white/40">[2025]</span>
          </motion.h1>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 }} className="max-w-4xl mx-auto px-6 lg:px-10 mb-16">
        <div className="rounded-2xl overflow-hidden h-72 md:h-96">
          <img src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=1200&q=85" alt="Shopify for Small Business" className="w-full h-full object-cover" style={{ filter: "brightness(0.65) saturate(0.9)" }} />
        </div>
      </motion.div>
      ...
      <article className="max-w-4xl mx-auto px-6 lg:px-10 pb-20">
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
          <p className="text-white/65 text-base leading-relaxed mb-5">
            In this article, we'll discuss whether you should consider Shopify as an ecommerce platform and whether Shopify is right for small businesses in 2025.
          </p>
        </motion.div>
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12">
          <h2 className="text-2xl font-black text-white mb-5 flex items-center gap-3">
            ...
            Why Choose Shopify for Your Small Online Store?
          </h2>
          <p className="text-white/60 text-base leading-relaxed mb-6">
            Shopify is the biggest selling point to be aware of: very few other platforms have Shopify's ability to offer merchants everything needed to sell online. They designed the platform with simplicity so you can set up an online store without needing to be a tech expert. The drag-and-drop website builder, clean navigation, and intuitive dashboard mean you can focus on your business, not on managing software.
          </p>
          <p className="text-white/60 text-base leading-relaxed mb-6">
            For any business owner, Shopify is to go-to when shopping for an ecommerce platform and increasing customer demand. Its ease of use makes it especially attractive to non-technical entrepreneurs.
          </p>

          {[
            { title: "App Store", body: "Shopify's biggest selling point is its ease of use. Many small business owners aren't tech-savvy, and they designed the platform with simplicity at its heart. Shopify also has an incredible app ecosystem. With over 7,000 apps in the marketplace, there is an integration for almost every business process — from inventory management and email marketing to customer loyalty and multi-channel selling." },
            { title: "Customer Support", body: "Shopify is a great option for all sizes of businesses, especially startups that often encounter challenges that may jeopardise their future if not quickly resolved. Shopify's support options extend beyond live chat — you get access to an extensive help centre, community forums, and a rich library of tutorials. Many merchants also praise the quality of their 24/7 support team." },
          ].map(({ title, body }, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="flex gap-5 p-5 rounded-2xl mb-4" style={cardStyle}>
              <span className="text-2xl font-black flex-shrink-0 leading-none mt-0.5" style={{ color: "rgba(124,58,237,0.35)" }}>{String(i + 1).padStart(2, "0")}</span>
              <div>
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12">
          <h2 className="text-2xl font-black text-white mb-5 flex items-center gap-3">
            <span className="w-1 h-6 rounded-full flex-shrink-0" style={{ background: GOLD }} />
            How Does Shopify Work for Small Businesses?
          </h2>
          <p className="text-white/60 text-base leading-relaxed mb-8">
            Getting started on Shopify is straightforward. You sign up, choose a plan, select a theme, and start adding products. Payment processing is built in through Shopify Payments, removing the need for a separate gateway. Here's how the key workflows look in practice:
          </p>

          {[
            { title: "Adding Products", body: "The product management interface is clean and intuitive. You can add descriptions, pricing, images, variants (size, colour, material), inventory levels, and shipping dimensions. Shopify also allows digital product downloads and service-based listings with no extra plugins." },
            { title: "Marketing and Analytics", body: "Shopify includes built-in SEO tools, email capture forms, discount codes, and integration with Google and Meta ad platforms. The analytics dashboard shows traffic sources, conversion rates, average order values, and top-performing products — giving you real data to act on, not just vanity metrics." },
          ].map(({ title, body }, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="flex gap-5 p-5 rounded-2xl mb-4" style={cardStyle}>
              <span className="text-2xl font-black flex-shrink-0 leading-none mt-0.5" style={{ color: "rgba(124,58,237,0.35)" }}>{String(i + 1).padStart(2, "0")}</span>
              <div>
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12">
          ...
          <p className="text-white/60 text-base leading-relaxed mb-6">
            Many small businesses have successfully used Shopify to scale their operations, reaching customers they never could have with a physical-only presence. From independent clothing brands and artisan food producers to niche subscription box services and handmade gift shops — Shopify powers over 1.7 million businesses across 175 countries.
          </p>
          <div className="grid md:grid-cols-3 gap-4">
            {[
              { label: "1.7M+", desc: "businesses using Shopify globally" },
              { label: "175", desc: "countries with active Shopify merchants" },
              { label: "£200B+", desc: "in total sales processed through Shopify" },
            ].map(({ label, desc }) => (
              <div key={label} className="text-center p-5 rounded-2xl" style={cardStyle}>
                <p className="text-3xl font-black mb-1.5" style={{ color: GOLD }}>{label}</p>
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-16 p-8 rounded-2xl" style={{ background: "linear-gradient(135deg, rgba(124,58,237,0.07) 0%, rgba(124,58,237,0.03) 100%)", border: "1px solid rgba(124,58,237,0.16)" }}>
          ...
          <p className="text-white/60 text-sm leading-relaxed mb-3">
            For most small businesses looking to sell online in 2025, Shopify is an excellent choice. Its ease of use, robust app ecosystem, reliable hosting, and quality support make it one of the most complete ecommerce platforms available at any price point.
          </p>
          <p className="text-white/60 text-sm leading-relaxed">
            At HireProgrammer, we help small and growing businesses make the most of platforms like Shopify — from custom theme development and app integrations to full Shopify migrations and ongoing optimisation. Book a free consultation to see how we can help your store perform better.
          </p>
        </motion.section>

        {/* tags */}
        <div className="flex flex-wrap gap-2 pb-8 border-b border-white/[0.06]">
          {["E-commerce", "Shopify", "Small Business", "Online Store", "eCommerce Platform", "UK Business"].map(t => (
